...
The normal process now is to configure the Alert to trigger this workflow when the alert fires. We will do that below (optionally), but first lets test our workflow.
...
2. Click on `self_healing_aiops` (if you don't still have it open)
...
<details>
  <summary>Click here to expand full updated workflow YAML</summary>
...
4. Click on the one alert - `	latency-threshold-alert-critical` (or the edit pencil)
    ![CleanShot 2025-11-14 at 12.59.56@2x.png](../assets/CleanShot%202025-11-14%20at%2012.59.56%402x.png)
...
---
Click **Next** to the Capstone challenge!